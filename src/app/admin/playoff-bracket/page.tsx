...
export default async function PlayoffBracketAdminPage() {
  ...
  const { count: unfinishedRegularSeasonGames } = await supabaseAdmin
    .from("games")
    .select("id", {
      count: "exact",
      head: true,
    ...
    <main className="mx-auto max-w-7xl px-4 py-10 text-white sm:px-6 lg:px-8">
      ...
      <section className="mt-4 rounded-[2rem] border border-[#C4963E]/30 bg-[#1A0F08]/90 p-6 shadow-2xl shadow-black/35 md:p-8">
        ...
        <h1 className="mt-2 text-4xl font-black">
          Generate Official Playoff Bracket
        </h1>

        <p className="mt-3 max-w-3xl leading-7 text-red-100/70">
          Use this after the regular season is locked. The generator creates a
          30-team double-elimination bracket from the current standings,
          excluding disqualified teams. Seeds #1 and #2 receive first-round
          BYEs.
        </p>
      </section>

      <div className="mt-8">
        <PlayoffBracketAdminClient
          seeds={seeds || []}
          games={games || []}
          unfinishedRegularSeasonGames={unfinishedRegularSeasonGames || 0}
        />
      </div>
    </main>
  );
